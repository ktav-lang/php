...
- Ключи в кавычках (§ 5.3.3) и escape-последовательность `\uXXXX` в
  inline-значениях (§ 3.7.1) появились в spec 0.7.0 и остаются доступны
  через ядро на Rust. MSRV — Rust 1.71; текущее значение
  `[package.metadata.ktav] spec-version` — "0.8.0".
