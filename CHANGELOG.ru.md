...
- **`Ktav::loadsStrict(string $src): mixed`** — strict-парсер доступен
  через PHP FFI и символ `ktav_loads_strict` в C ABI.
...
- Литеральный backslash внутри ключа теперь требует `\\` (раньше `\` в
  ключе считался обычным байтом). На практике встречается редко; по
  pre-1.0 SemVer — MINOR bump.
...
- Типизированные маркеры `:i` / `:f` удалены. Числа, булевы и `null`
  определяются по лексической форме (spec §§ 3.6, 5.2). Пишите
  `port: 8080` для Integer, `port:: 8080` для сохранения String.
- Комментарии теперь начинаются с `##`. Одиночный `#` — это контент.
- Голые целые и дробные числа больше не парсятся как String.
- Пробелы в начале/конце ключей теперь обрезаются.

### Добавлено

- **Инлайн-компоунды** `{k: v, …}` / `[i, …]` (spec § 5.8).
- **Восемь escape-последовательностей** в инлайн-скалярах (spec § 3.7).
- **`Ktav::emitCanonical($value)`** — рендер в детерминированную
  каноническую форму Ktav (spec § 7).

### Изменено

- Лицензия: MIT → MIT OR Apache-2.0 (`LICENSE-MIT` + `LICENSE-APACHE`).
- Субмодуль spec: v0.5.0.
